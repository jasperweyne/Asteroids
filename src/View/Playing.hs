module View.Playing (viewPlaying) where
  ...
  renderObjects gs = Pictures (map render (asteroids gs) ++ map render (saucers gs) ++ map render (rockets gs) ++ [render (player gs)])

  renderHud :: InGameState -> Picture
  renderHud gs@InGameState{player = p@Player{lives = l}, asteroids = as, saucers = s, rockets = r} = translate 350 350 $ color white $ pictures [
